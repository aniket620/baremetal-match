"""
Agent definitions for BareMetal Match.
...
# =============================================================================
# COMPATIBILITY PATCH: newer Claude models (this session's crewai==0.203.1
# predates the fix) reject a conversation that ends with an "assistant" role
# message as an unsupported prefill attempt. CrewAI's ReAct loop appends the
# tool observation as another assistant-role message, which trips this on
# every tool call. CrewAI already works around the equivalent issue for
# Mistral/Ollama by inserting a synthetic trailing user message; this applies
# the same fix for Anthropic models. See:
# https://github.com/crewAIInc/crewAI/issues/4798
# =============================================================================

_original_format_messages_for_provider = _CrewLLM._format_messages_for_provider


def _format_messages_for_provider_patched(self, messages):
    messages = _original_format_messages_for_provider(self, messages)
    if self.is_anthropic and messages and messages[-1]["role"] == "assistant":
        return [*messages, {"role": "user", "content": "Please continue."}]
    return messages


_CrewLLM._format_messages_for_provider = _format_messages_for_provider_patched
...
llm = LLM(
    model=CLAUDE_MODEL,
    # Note: `temperature` is intentionally omitted — claude-sonnet-5 rejects
    # this parameter ("temperature is deprecated for this model").
)
