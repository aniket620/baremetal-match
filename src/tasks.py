"""
Task definitions for BareMetal Match.
...
def create_job_search_task(
    agent, role: str, location: str, num_results: int, target_skills: str = ""
) -> Task:
    """
    ...
    Args:
        ...
        target_skills: Optional comma-separated list of specific skills the
            candidate wants to match against (e.g. "C, C++, RTOS, CAN")
    ...
    target_skills_note = (
        f"""
<target_skills>
The candidate is specifically targeting roles that use: {target_skills}.
The "what" search term ("role") must stay a short, natural job title — do NOT
stuff these individual skills into the Job Search Tool's "role" input, since
Adzuna's search matches ALL words literally and a long keyword list will
return zero results. Instead, after retrieving results, call out in your
summary which of these target skills appear in which listings.
</target_skills>
"""
        if target_skills
        else ""
...
def create_skills_analysis_task(
    agent, job_search_task: Task, role: str, target_skills: str = ""
) -> Task:
    """
    ...
    Args:
        ...
        target_skills: Optional comma-separated list of specific skills the
            candidate already has or wants evaluated (e.g. "C, C++, RTOS, CAN")
...
The candidate specifically has or wants to be evaluated against these skills:
{target_skills}.
For EACH of these, explicitly state: (a) whether it appeared in the job
listings and how often, (b) how critical it is for {role} roles even if a
given listing didn't mention it, and (c) any gaps between this list and what
...
def create_interview_prep_task(
    ...
    Args:
        ...
        target_skills: Optional comma-separated list of specific skills to
            drill on in the technical questions (e.g. "SPI, I2C, RTOS, CAN")
    ...
    target_skills_note = (
        f"""
<candidate_target_skills>
The candidate wants technical questions specifically covering: {target_skills}.
Make sure at least one technical question drills into each of these skills
directly (protocol-level, debugging, or design-tradeoff questions), even if a
specific job listing didn't explicitly mention it.
</candidate_target_skills>
"""
        if target_skills
        else ""
...
def create_career_advisory_task(
    ...
    Args:
        ...
        target_skills: Optional comma-separated list of specific skills to
            make sure are represented as resume/LinkedIn keywords
    ...
    target_skills_note = (
        f"""
<candidate_target_skills>
The candidate specifically has these skills: {target_skills}.
Make sure the resume keyword list and LinkedIn skills section explicitly
include each of these (using industry-standard naming/capitalization,
e.g. "I2C", "SPI", "CAN bus", "J1939"), and call out how to phrase them
...
def create_all_tasks(
    ...
    Args:
        ...
        target_skills: Optional comma-separated list of specific skills the
            candidate wants every downstream agent to evaluate against
            (e.g. "C, C++, Python, SPI, I2C, RTOS, CAN, J1939")
